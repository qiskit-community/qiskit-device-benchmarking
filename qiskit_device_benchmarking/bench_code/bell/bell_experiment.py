...
def get_bell_layers(backend,crs=None, qubits=None, min_sep=2):
    """
    Determine which gates can be calibrated in parallel based on separation
    and relative gate widths. Set `max_width_diff = None` for separation_only
    like behavior.
    """
    if crs is None:
        crs = backend.configuration().coupling_map
    if qubits:
        # filter the CR gate list by the list of qubits
        import numpy as np

        if not isinstance(qubits, list):
            qubits = list(qubits)
        _qubits = np.array(qubits)
        _crs = np.array(crs)

        # create a bool filter and return a reduced set of crs
        X = np.isin(_crs, _qubits)  # 2d
        # combine 2d colums with OR and return a filtered crs list of tuples
        crs = [list(i) for i in _crs[X[:, 0] | X[:, 1]]]

    cr_width_dict = dict()  # cache widths for speed
    cr_sets = []

    busses = backend.configuration().coupling_map # get_busses()
    for cr in crs:
        # grow a neighborhood of qubits around the ones involved in the gate
        cr_nbhd = set(cr)
        for _ in range(min_sep):
            cr_nbhd |= set(
                [bq for b in busses for bq in b if b[0] in cr_nbhd or b[1] in cr_nbhd]
            )
        # check if any existing cr sets include any qubits from the neighborhood
        # also don't put gates of significantly different widths together
        placed = False
        for cr_set in cr_sets:
            if (cr in cr_set):  # would only be true if cr_calsets was manually specified in device.yaml
                placed = True
                break
            conflict = False
            for cr_gate in cr_set:
                normalized_width_difference = None
                if (
                    cr_gate[0] in cr_nbhd
                    or cr_gate[1] in cr_nbhd
                ):
                    conflict = True
            if not conflict:
                cr_set.append(cr)
