...
class MirrorQuantumVolumeAnalysis(BaseAnalysis):
    ...
    def _run_analysis(self, experiment_data):
        ...
        for data_trial in data:
            ...
            if self.options.calc_hop:
                ...
                print(data_trial["metadata"])
                
            if data_trial["metadata"]["is_mirror_circuit"]:
                target_bitstring = data_trial["metadata"]["target_bitstring"]
                success_probs.append(self._calc_success_probability(data_trial, target_bitstring))

        if self.options.calc_hop:
            hop_result, qv_result = self._calc_quantum_volume(heavy_output_prob_exp, depth, num_trials)
            results = [hop_result, qv_result]
        else:
            results = []
            
        if data[0]["metadata"]["is_mirror_circuit"]:
            success_prob_result = AnalysisResultData(
                "mean_success_probability",
                value=uncertainties.ufloat(
                    nominal_value=np.mean(success_probs), std_dev=np.std(success_probs)
                ),
                quality="good",
                extra={
                    "depth": depth,
        ...
        if (
            data[0]["metadata"]["is_mirror_circuit"]
            and not data[0]["metadata"]["has_middle_paulis"]
        ):
    ...
    @staticmethod
    def _calc_success_probability(data, target_bitstring):
        """
        Calculate the success probability from circuit data

        Args:
            data (dict): the result of the circuit execution
            target_bistring (string): the expected bitstring output of the circuit
        ...
        # Number of shots in which target bit string is observed
        target_counts = data["counts"].get(target_bitstring, 0)

        return target_counts / circ_shots
